...
public class Gunnumscript : MonoBehaviour
{
    Text textcomponent;
    ...
    {
        //テキストの処理
        textcomponent.text = "弾を打った数：" + Gamemanager.instance.gunnum + "回";

        
        //フェイドインの処理
        if (Gamemanager.instance.FinishedTimer > finishbackgroundscript.resulttime + 1 && !isseen)
        {
            textcomponent.color = new Color(0, 0, 0, Gamemanager.instance.FinishedTimer - finishbackgroundscript.resulttime - 1);
            if (Gamemanager.instance.FinishedTimer - finishbackgroundscript.resulttime > 2)
            {
                isseen = true;
            }
        }
